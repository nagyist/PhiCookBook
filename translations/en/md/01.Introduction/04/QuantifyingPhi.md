...
We hope to deploy GenAI models to edge devices and allow more devices to enter GenAI scenarios, such as mobile devices, AI PC/Copilot+PC, and traditional IoT devices. Through the quantized model, we can deploy it to different edge devices based on the device. Combined with the model acceleration framework and quantized model provided by hardware manufacturers, we can build better SLM application scenarios.
...
On mobile devices or IoT devices, we can convert Phi-3.x models to INT4, while AI PC / Copilot PC can use higher precision such as INT8, FP16, FP32.
